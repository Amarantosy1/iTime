...
struct StarrySkyBackground: View {
    ...
    private func drawMeteors(on canvas: GraphicsContext, size: CGSize, time: TimeInterval) {
        for meteor in meteors {
            ...
            if cycle < 0.1 || cycle > 0.92 {
                continue
            }

            let progress = (cycle - 0.1) / 0.82
            let headX = (progress * 1.35 - 0.2) * size.width
            let headY = (meteor.laneY + progress * 0.35) * size.height

            let tailX = headX - meteor.length
            let tailY = headY - meteor.length * 0.38
            ...
            canvas.stroke(
                path,
                with: .linearGradient(
                    Gradient(colors: [.white.opacity(meteor.opacity), .white.opacity(0.0)]),
                    startPoint: head,
                    endPoint: tail
                ),
            ...
            canvas.fill(Path(ellipseIn: headRect), with: .color(.white.opacity(min(1.0, meteor.opacity + 0.2))))
        }
    }
